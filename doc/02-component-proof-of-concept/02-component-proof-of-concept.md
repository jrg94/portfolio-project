...
A second factor is representation simplicity. A 2D Tensor can be backed
by a single flat `double[]` array in row-major order, with a pair of `int`
fields for shape. There are no recursive types, no internal helper classes,
and no complex invariants to maintain — making it a realistic single-file
proof-of-concept.

Third, the Tensor design offers method variety that is ideal for
demonstrating feasibility. The kernel layer: `get`, `set`, `shape`, `isZero` covers
basic access and query, while the secondary layer: `add`, `scale`, `reshape`, `sum`,
`multiply`, `fill` ranges from simple element-wise operations all the way to
O(n³) matrix multiplication. This spread proves the component can support
both trivial and non-trivial operations.

In comparison, ComplexNumber is mathematically elegant but too simple
(two `double` fields, limited demo scenarios), and Vector is essentially
a 1D special case of Tensor — building Tensor first provides the stronger
and more general foundation. Both alternatives would have resulted in a less
